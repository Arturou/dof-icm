"""Single-process Air application for controlled human evaluation of the DOF-ICM agent.

Adapted from ``CodeandoGuadalajara/dof-rag/human_eval`` (MIT). The only
backend change: runs are answered by the DOF-ICM file-based agent
(``icm_executor.IcmRunExecutor``) over the plain-markdown corpus instead of
the upstream ``agent_tools`` + ``dof_db`` vector stack. Every UI behaviour
(auth, quota, review gate, publish queue, feedback) is preserved.

Authentication is provider-agnostic: ``create_app`` receives an
``auth.AuthBackend`` and never imports a provider. ``build_default_app`` picks
the backend from ``DOF_AUTH_BACKEND``:

- ``clerk`` — multi-user accounts + roles (requires Clerk env vars), as in the
  upstream site;
- ``local`` (default without Clerk) — single-admin password sign-in for a
  self-hosted instance (``DOF_LOCAL_PASSWORD``);
- ``header`` — header-driven users for tests/scripts (``X-Eval-User``).
...
- admins (Clerk ``public_metadata.role == "admin"``, or the local admin)
  publish/unpublish answers and are exempt from the quota and the feedback
  gate.
"""
...
from .auth import AuthBackend, LocalAuthBackend, User
...
class WebSettings:
    ...
    def from_env(cls, repo_root: Path) -> "WebSettings":
        ...
        if session_secret and len(session_secret) < 32:
            raise ValueError("DOF_SESSION_SECRET must be at least 32 characters")
        allowed_hosts = tuple(
            value.strip()
        ...
        return cls(
            ...
            auth_backend=os.environ.get("DOF_AUTH_BACKEND", "auto").lower(),
...
Las respuestas publicadas son públicas. No registramos direcciones IP.</footer>
...
<p class="lede">El agente busca directamente en el corpus de archivos del DOF (2024–2026) y cita cada
afirmación con su documento y líneas exactas. Las respuestas revisadas por el equipo
...
Las cuentas y los roles dependen del backend de autenticación configurado (Clerk o local).</p></section>
...
def create_app(
    ...
    *,
    ...
    local_backend = getattr(auth_backend, "local", False)

    def _local_login_page(
        request: Request, target: str, *, error: str | None = None
    ) -> HTMLResponse:
        configured = bool(getattr(auth_backend, "password", ""))
        error_html = (
            f'<p class="warning" role="alert">{_escape(error)}</p>' if error else ""
        )
        if not configured:
            body = (
                ...
                '<p class="lede">La instancia local no tiene contraseña '
                'configurada. Define <code>DOF_LOCAL_PASSWORD</code> para '
                "habilitar el acceso.</p></section>"
            )
        else:
            body = f"""<section class="panel login-panel"><h2>Entrar</h2>{error_html}
<p class="lede">Instancia autohospedada (DOF-ICM). Usa la contraseña definida en
<code>DOF_LOCAL_PASSWORD</code>.</p>
<form method="post" action="/login">
<input type="hidden" name="csrf_token" value="{_escape(_csrf(request))}">
<input type="hidden" name="next" value="{_escape(target)}">
...
<button type="submit">Entrar</button></form></section>"""
    ...
    async def login(request: Request, next: str = "/") -> Response:
        ...
        # stays provider-agnostic; without it the page explains the local or
        # header-based login instead.
        ...
        if login_scripts is None:
            if local_backend:
                return _local_login_page(request, target)
            body = (
                '<section class="panel login-panel"><h2>Entrar</h2>'
    ...
    if local_backend:
        ...
        async def local_login(request: Request) -> Response:
            ...
            if not _csrf_valid(request, form.get("csrf_token")):
                return HTMLResponse(
                    "La sesión del formulario venció.", status_code=403
                )
            target = _sanitize_login_next(form.get("next", "/"))
            if auth_backend.check_password(form.get("password")):
                auth_backend.sign_in(request)
                return RedirectResponse(target, status_code=303)
            return _local_login_page(
                request, target, error="Contraseña incorrecta."
            )
        ...
        async def local_logout(request: Request) -> Response:
            ...
            auth_backend.sign_out(request)
            return RedirectResponse(
                _safe_next(form.get("next"), "/"), status_code=303
            )
    elif login_scripts is None:
        # Header-driven dev backend: nothing to sign out of, but the page
        # renders a Salir button for any identified user.
        @app.post("/logout", response_class=HTMLResponse)
        async def header_logout(request: Request) -> Response:
            form = await _form(request)
            if not _csrf_valid(request, form.get("csrf_token")):
                return HTMLResponse(
                    "La sesión del formulario venció.", status_code=403
...
def build_default_app(repo_root: Path | None = None) -> tuple[Any, WebSettings]:
    ...
    # Without an explicit DOF_SESSION_SECRET the app signs session cookies
    # with an ephemeral key: safe for a local instance (sessions reset on
    # restart), but set a fixed secret for anything long-lived/public.
    if not settings.session_secret:
        settings = replace(
            settings, session_secret=secrets.token_urlsafe(48)
        )
        LOGGER.warning(
            "DOF_SESSION_SECRET not set: using an ephemeral session secret "
            "(sessions reset when the app restarts)"
        )

    auth_mode = settings.auth_backend
    if auth_mode == "auto":
        # auto: Clerk when its env is present (multi-user), else local.
        auth_mode = "clerk" if os.environ.get("CLERK_SECRET_KEY") else "local"
    auth_backend: AuthBackend
    page_scripts: Callable[[User | None], str] | None = None
    login_scripts: Callable[[str], str] | None = None
    include_airclerk = False
    if auth_mode == "clerk":
        # Imported lazily: airclerk validates Clerk environment variables at
        # import time, so Clerk-only deployments never import it.
        import airclerk
        ...
        auth_backend = ClerkAuthBackend(
            secret_key=airclerk.settings.CLERK_SECRET_KEY
        )
        page_scripts = clerk_page_scripts
        login_scripts = clerk_login_scripts
        include_airclerk = True
    elif auth_mode == "header":
        from .auth import FakeAuthBackend

        auth_backend = FakeAuthBackend()  # type: ignore[assignment]
    elif auth_mode == "local":
        auth_backend = LocalAuthBackend(  # type: ignore[assignment]
            email=os.environ.get("DOF_LOCAL_EMAIL", "admin@local"),
            password=os.environ.get("DOF_LOCAL_PASSWORD", ""),
        )
    else:
        raise ValueError(
            "DOF_AUTH_BACKEND must be one of: clerk, local, header, auto"
        )

    executor = IcmRunExecutor(IcmExecutorConfig.from_env(root))
    service = EvaluationService(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Serve the DOF-ICM human-evaluation site"
    )
    parser.add_argument(
